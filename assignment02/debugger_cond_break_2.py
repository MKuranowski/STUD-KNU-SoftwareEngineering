...
class Debugger(Tracer):
    ...
    def __init__(self, *, file: TextIO = sys.stdout) -> None:
        ...
        self.break_expressions: list[str] = []
    ...
    def eval_condition(self, cond: str) -> bool:
        try:
            return bool(eval(cond, self.local_vars, self.frame.f_globals))
        except NameError:
            return False  # don't break if something is not defined

    def stop_here(self) -> bool:
        """Return True if we should stop"""
        return (
            self.stepping
            or self.frame.f_lineno in self.breakpoints
            or any(self.eval_condition(expr) for expr in self.break_expressions)
        )
    ...
    def condbreak_command(self, arg: str) -> None:
        """Break execution while an expression is true. Without an argument, list conditional breakpoints."""
        if arg:
            self.break_expressions.append(arg)
        self.log("Break conditions:", self.break_expressions)
